...
/*   Updated: 2025/02/22 15:40:50 by ysumeral         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_map_rectangular(t_data *data, char *line)
{
	int	x;

	x = ft_strlen(line);
	if (x != data->map.size_x)
...
static int	check_map(t_data *data)
{
	...
	while (y < data->map.size_y && data->map.map[y] != NULL)
	{
		if (!data->map.map || y >= data->map.size_y)
			return (error_handler(ERROR_MAP_WALLS));
		if (check_map_rectangular(data, data->map.map[y]))
			return (1);
